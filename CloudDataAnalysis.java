/*public class CloudDataAnalysis {
   static float xAverage, yAverage; // Prevailing wind average
   private static void prevailingWindAve (CloudData data) {
      float sumX = 0;
      float sumY = 0;
      Vector [][][] wind = data.getAdvection();
      float[] myAverages = data.getAverages();
      ...
      xAverage = (float) sumX/data.dim();
      yAverage = (float)sumY/data.dim();
   }
   
   private static float localAve (CloudData data, int t, int i, int j) {
   ...
   private static void classify (CloudData data) {
      float [][][] convection = data.getConvection();
      int [][][] classification = data.getClassification();
      for(int s = 0; s < data.timestep(); s++){
          for(int a = 0; a < data.width(); a++){
              for(int b = 0; b < data.height(); b++){
                  float windMagnitude = localAve (data, s, a, b);
                  // classify
